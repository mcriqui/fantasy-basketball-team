...
def todays_date(request):
    ...
    wizards_home = Game.objects.filter(home__city="Washington").filter(away__in=players_teams).filter(played_on_date__contains=datetime.date(2014, 12, 21))
    wizards_away = Game.objects.filter(away__city="Washington").filter(home__in=players_teams).filter(played_on_date__contains=datetime.date(2014, 12, 25))
